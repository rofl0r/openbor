...
void pp_message(pp_parser * self, char *messageType, char *message) {
	...
	printf("Script %s: %s, line %d: %s\n", messageType, parser->filename, parser->lexer.theTokenPosition.row,
	       message);
 ...
 * @return E_FAIL
 */
s32 pp_error(pp_parser * self, char *format, ...) {
	char buf[1024] = { "" };
	va_list arglist;
 ...
 */
s32 pp_parser_lex_token(pp_parser * self, bool skip_whitespace) {
	bool success = true;
 ...
 * @param skip_whitespace true to ignore whitespace, false otherwise
 */
s32 pp_parser_lex_token_essential(pp_parser * self, bool skip_whitespace) {
	pp_parser *parser = self;
		   ...
		   !(self->ctx->conditionals.top == cs_false || self->ctx->conditionals.top == cs_done)) {
			...
			switch (self->token.theType) {
					...
					if(self->type == PP_FUNCTION_MACRO) {
						...
						if(self->token.theType == PP_TOKEN_IDENTIFIER &&
						   List_FindByName(&self->ctx->macros, self->token.theSource)) {
							...
							self->overread = true;
					} else if(self->newline) {	/* only parse the "#" symbol when it's at the beginning of a
				...
				case PP_TOKEN_IDENTIFIER:
					...
					if(self->token.theType == PP_TOKEN_LPAREN
...
s32 pp_parser_readline(pp_parser * self, char *buf, int bufsize) {
	int total_length = 1;
	...
	while(1) {
		...
		if((total_length + strlen(self->token.theSource)) > bufsize) {
			...
			pp_error(self, "length of macro or message contents is too long; must be <= %i characters",
				 bufsize);
			return E_FAIL;
		}
 ...
 */
s32 pp_parser_stringify(pp_parser * self) {
	TEXTPOS lexerPosition = { 1, 0 };
	char *contents = (char *) List_Retrieve(&self->ctx->macros);
	pp_parser parser;
	pp_token *token;

	pp_token_Init(&self->token, PP_TOKEN_STRING_LITERAL, "\"", self->token.theTextPosition, 0);
 ...
 */
s32 pp_parser_parse_directive(pp_parser * self) {
	if(FAILED(pp_parser_lex_token(self, true)))
		return E_FAIL;
	...
	if(self->ctx->conditionals.top == cs_false || self->ctx->conditionals.top == cs_done) {
		if(self->token.theType != PP_TOKEN_ELIF &&
		   self->token.theType != PP_TOKEN_ELSE && self->token.theType != PP_TOKEN_ENDIF) {
			return S_OK;
		}
	}
 ...
 */
s32 pp_parser_include(pp_parser * self, char *filename) {
	char *buffer;
	int length;
	int bytesRead;
	int handle;
 ...
 */
s32 pp_parser_define(pp_parser * self, char *name) {
	char *contents = NULL;
		...
		while(self->token.theType != PP_TOKEN_RPAREN) {
			switch (self->token.theType) {
				...
				case PP_TOKEN_ERROR_TEXT:
					...
					if(FAILED(pp_parser_lex_token(self, true)))
						goto error;
					if(self->token.theType == PP_TOKEN_COMMA
					   || self->token.theType == PP_TOKEN_RPAREN)
						break;
				default:
					return pp_error(self, "unexpected token '%s' in #define parameter list",
							self->token.theSource);
			}
 ...
 */
s32 pp_parser_conditional(pp_parser * self, PP_TOKEN_TYPE directive) {
	switch (directive) {
		case PP_TOKEN_IF:
			return pp_error(self, "#if directive not supported");
		case PP_TOKEN_IFDEF:
		case PP_TOKEN_IFNDEF:
			if(self->ctx->num_conditionals++ > 16)
		...
		case PP_TOKEN_ELIF:
			...
			if(self->ctx->conditionals.top == cs_done || self->ctx->conditionals.top == cs_true)
				...
				self->ctx->conditionals.top =
				    pp_parser_eval_conditional(self, directive) ? cs_true : cs_false;
			break;
#endif
		case PP_TOKEN_ELSE:
...
bool pp_parser_eval_conditional(pp_parser * self, PP_TOKEN_TYPE directive) {
	...
	if(FAILED(pp_parser_lex_token(self, true)))
		return false;	// we can deal with lexer errors later!
 ...
 */
s32 pp_parser_insert_function_macro(pp_parser * self, char *name) {
	...
	do {
		...
		switch (self->token.theType) {
			case PP_TOKEN_NEWLINE:
				if(paramBuffer[0] != '\0')
					return pp_error(self, "unexpected newline in parameter list for function '%s'",
							name);
			case PP_TOKEN_EOF:
				return pp_error(self, "unexpected end of file in parameter list for function '%s'",
						name);
			...
			case PP_TOKEN_COMMA:
				...
				if(strncmp(paramBuffer, List_GetName(params), sizeof(paramBuffer)) != 0) {
					...
					paramMacros++;
					List_InsertBefore(&self->ctx->macros, strdup(paramBuffer),
							  List_GetName(params));
				}
