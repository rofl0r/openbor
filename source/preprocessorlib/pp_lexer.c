...
s32 pp_lexer_GetTokenIdentifier(pp_lexer * plexer, pp_token * theNextToken) {
	...
	do {
		...
		(*plexer->pcurChar >= 'a' && *plexer->pcurChar <= 'z') ||
		(*plexer->pcurChar >= 'A' && *plexer->pcurChar <= 'Z') || (!strncmp(plexer->pcurChar, "_", 1)));
...
s32 pp_lexer_SkipComment(pp_lexer * plexer, COMMENT_TYPE theType) {
	...
	if(theType == COMMENT_SLASH) {
		do {
			...
			if(!strncmp(plexer->pcurChar, "\\\n", 2) ||
			   !strncmp(plexer->pcurChar, "\\\r", 2) || !strncmp(plexer->pcurChar, "\\\f", 2)) {
				SKIPCHARACTER;
				plexer->theTextPosition.col = 0;
				plexer->theTextPosition.row++;
				continue;
			...
			if(!strncmp(plexer->pcurChar, "\r\n", 2) ||
			   ...
			   !strncmp(plexer->pcurChar, "\r", 1) || !strncmp(plexer->pcurChar, "\f", 1)) {
				break;
			}
