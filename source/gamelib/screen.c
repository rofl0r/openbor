...
s_screen *allocscreen(int width, int height, int pixelformat) {
	...
	if(pixelformat == PIXEL_x8)
		screen =
		    (s_screen *) malloc(sizeof(s_screen) + width * height * pixelbytes[(int) pixelformat] + PAL_BYTES +
					ANYNUMBER);
	else
		screen =
		    (s_screen *) malloc(sizeof(s_screen) + width * height * pixelbytes[(int) pixelformat] + ANYNUMBER);
	if(screen == NULL)
		return NULL;
	screen->width = width;
...
void copyscreen_water(s_screen * dest, s_screen * src, int x, int y, int amplitude, float wavelength, int time,
		      int watermode) {
	unsigned char *sp, *dp;
	int pixelformat = src->pixelformat;
	int sw = src->width;
...
// Screen copy function with offset options. Supports clipping.
void copyscreen_trans_water(s_screen * dest, s_screen * src, int x, int y, int amplitude, float wavelength, int time,
			    int watermode) {
	unsigned char *sp, *dp;
	//int pixelformat = src->pixelformat;
	int sw = src->width;
...
// Screen copy function with offset options. Supports clipping.
void blendscreen_water(s_screen * dest, s_screen * src, int x, int y, int amplitude, float wavelength, int time,
		       int watermode, unsigned char *lut) {
	unsigned char *sp, *dp;
	//int pixelformat = src->pixelformat;
	int sw = src->width;
	int sh = src->height;
...
void putscreen(s_screen * dest, s_screen * src, int x, int y, s_drawmethod * drawmethod) {
	...
	if(dest->pixelformat == PIXEL_8) {
		...
		if(src->pixelformat == PIXEL_x8)
			putscreenx8p16(dest, src, x, y, transbg, (unsigned short *) table,
				       alpha > 0 ? blendfunctions16[alpha - 1] : NULL);
		...
		if(src->pixelformat == PIXEL_x8)
			putscreenx8p32(dest, src, x, y, transbg, (unsigned *) table,
				       alpha > 0 ? blendfunctions32[alpha - 1] : NULL);
		else if(src->pixelformat == PIXEL_32)
			blendscreen32(dest, src, x, y, transbg, alpha > 0 ? blendfunctions32[alpha - 1] : NULL);
	}
}

void putscreen_water(s_screen * dest, s_screen * src, int x, int y, int amplitude, float wavelength, int time,
		     int watermode, s_drawmethod * drawmethod) {
	unsigned char *table;
	int alpha, transbg;
	if(!drawmethod || drawmethod->flag == 0) {
		table = NULL;
	...
	if(dest->pixelformat == PIXEL_8) {
		if(alpha > 0) {
			blendscreen_water(dest, src, x, y, amplitude, wavelength, time, watermode,
					  blendtables[drawmethod->alpha - 1]);
		} else if(transbg)
			copyscreen_trans_water(dest, src, x, y, amplitude, wavelength, time, watermode);
		else
			copyscreen_water(dest, src, x, y, amplitude, wavelength, time, watermode);
	} else if(dest->pixelformat == PIXEL_16) {
		putscreenx8p16_water(dest, src, x, y, transbg, (unsigned short *) table,
				     alpha > 0 ? blendfunctions16[alpha - 1] : NULL, amplitude, wavelength, time,
				     watermode);
	} else if(dest->pixelformat == PIXEL_32) {
		putscreenx8p32_water(dest, src, x, y, transbg, (u32 *) table,
				     alpha > 0 ? blendfunctions32[alpha - 1] : NULL, amplitude, wavelength, time,
				     watermode);
	}
}
