	...
	for(i = 0; i < spritequeue_len; i++) {
		...
		switch (order[i]->type) {
			...
			case SQT_SCREEN:	// draw a screen instead of sprite
				putscreen(screen, (s_screen *) (order[i]->frame), order[i]->x, order[i]->y,
					  &(order[i]->drawmethod));
				break;
			case SQT_DOT:
				switch (screen->pixelformat) {
					case PIXEL_8:
						putpixel(order[i]->x, order[i]->y, order[i]->params[0], screen,
							 order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
					case PIXEL_16:
						putpixel16(order[i]->x, order[i]->y, order[i]->params[0], screen,
							   order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
					case PIXEL_32:
						putpixel32(order[i]->x, order[i]->y, order[i]->params[0], screen,
							   order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
				}
				break;
			case SQT_LINE:
				switch (screen->pixelformat) {
					case PIXEL_8:
						line(order[i]->x, order[i]->y, order[i]->params[1], order[i]->params[2],
						     order[i]->params[0], screen,
						     order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
					case PIXEL_16:
						line16(order[i]->x, order[i]->y, order[i]->params[1],
						       order[i]->params[2], order[i]->params[0], screen,
						       order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
					case PIXEL_32:
						line32(order[i]->x, order[i]->y, order[i]->params[1],
						       order[i]->params[2], order[i]->params[0], screen,
						       order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
				}
			...
			case SQT_BOX:
				switch (screen->pixelformat) {
					case PIXEL_8:
						drawbox(order[i]->x, order[i]->y, order[i]->params[1],
							order[i]->params[2], order[i]->params[0], screen,
							order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
					case PIXEL_16:
						drawbox16(order[i]->x, order[i]->y, order[i]->params[1],
							  order[i]->params[2], order[i]->params[0], screen,
							  order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
					case PIXEL_32:
						drawbox32(order[i]->x, order[i]->y, order[i]->params[1],
							  order[i]->params[2], order[i]->params[0], screen,
							  order[i]->drawmethod.flag ? order[i]->drawmethod.alpha : 0);
						break;
				}
