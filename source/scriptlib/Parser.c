...
void Parser_External_decl2(Parser * pparser, bool variableonly) {
	...
	else if(variableonly == FALSE
		&& ParserSet_First(&(pparser->theParserSet), funcDecl, pparser->theNextToken.theType)) {
...
void Parser_Iter_stmt(Parser * pparser) {
	...
	if(Parser_Check(pparser, TOKEN_WHILE)) {
		...
		if(pDefInst) {
			...
			PFOREACH(pDefInst, pInstruction = (Instruction *) List_Retrieve(pDefInst);
				 /*
