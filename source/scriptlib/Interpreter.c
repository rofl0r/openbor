...
void Interpreter_Clear(Interpreter * pinterpreter) {
	...
	if(pinterpreter->theSolidListOfInstructionList) {
		...
		FOREACH(pinterpreter->theInstructionList,
			...
			Instruction_Clear(pinstruction); free((void *) pinstruction); pinstruction = NULL;);
	}
...
s32 Interpreter_Call(Interpreter * pinterpreter) {
	...
	if(currentCall->ptheJumpTarget) {
		...
		hr = currentCall->functionRef(currentCall->theSolidListOfRefList ? (ScriptVariant **) currentCall->
					      theSolidListOfRefList->solidlist : NULL, &(pretvar),
					      (int) currentCall->theRef->lVal);
		if(FAILED(hr)) {
		...
		for(index = 0; index < size; index++) {
			...
			if(bImmediate) {
				pinterpreter->pCurrentInstruction =
				    ((Instruction **) pinterpreter->theSolidListOfInstructionList->solidlist) + index;
...
s32 Interpreter_CompileInstructions(Interpreter * pinterpreter) {
	...
	for(i = 0; i < size; i++) {
		...
		switch (pInstruction->OpCode) {
			...
			case LOAD:
				hr = Interpreter_GetValueByRef(pinterpreter, pInstruction->theToken->theSource,
							       &(pInstruction->theRef));
				// cache value
			...
			case CALL:
				...
				if(List_FindByName(&(pinterpreter->theInstructionList), pToken->theSource)) {
					pInstruction->theJumpTargetIndex =
					    List_GetIndex(&(pinterpreter->theInstructionList));
					...
					pInstruction->functionRef =
					    (SCRIPTFUNCTION) List_Retrieve(pinterpreter->ptheFunctionList);
				...
				if(((Instruction *) List_Retrieve(&(pinterpreter->theInstructionList)))->OpCode !=
				   CLEAN) {
		...
		if(FAILED(hr)) {
			printf("\nScript compile error in '%s': %s line %d, column %d\n",
			       pinterpreter->theSymbolTable.name,
			       (pInstruction->theToken) ? pInstruction->theToken->theSource : "",
	...
	if(pinterpreter->mainEntryIndex >= 0)
		pinterpreter->pMainEntry =
		    (Instruction
		     **) (&(pinterpreter->theSolidListOfInstructionList->solidlist[pinterpreter->mainEntryIndex]));
	else
		...
		if(pInstruction->theJumpTargetIndex >= 0)
			pInstruction->ptheJumpTarget =
			    (Instruction **) & (pinterpreter->theSolidListOfInstructionList->
						solidlist[pInstruction->theJumpTargetIndex]);
		else
			...
			case PARAM:
				if(pinterpreter->pCurrentCall) {
					...
					ScriptVariant_Copy(pInstruction->theVal,
							   (ScriptVariant *) (currentCall->theSolidListOfRefList->
									      solidlist[currentCall->theRefList->
											index]));
			...
			case CHECKARG:
				if(pinterpreter->pCurrentCall) {
					...
					if(pInstruction->theVal->lVal != currentCall->theRef->lVal) {
						printf
						    ("Runtime error: argument count(%d) doesn't match, check your function call: %s.\n",
...
void Interpreter_OutputPCode(Interpreter * pinterpreter, const char* fileName) {
	...
	FOREACH(pinterpreter->theInstructionList,
		...
		//buffer
		pLabel = List_GetName(&(pinterpreter->theInstructionList)); if(pLabel != NULL)
		strcpy(buffer, pLabel); strcat(buffer, "\t");
		//Get the pseudo-assembly representation of the CInstruction and concat
		//it onto the output buffer
		pStr = (char *) malloc(256); Instruction_ToString(pInstruction, (char *) pStr); strcat(buffer, pStr);
		//donot forget to delete it
		free((void *) pStr); strcat(buffer, "\n");
