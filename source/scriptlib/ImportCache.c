...
s32 ImportNode_Init(ImportNode * node, const char *path) {
	...
	if(scriptText == NULL)
		goto error;
	if(FAILED(Interpreter_ParseText(&node->interpreter, scriptText, 1, path)))
		goto error;
	if(FAILED(Interpreter_CompileInstructions(&node->interpreter)))
		goto error;
	free(scriptText);
	return S_OK;
...
ImportNode *ImportCache_Retrieve(const char *path) {
	...
	} else			// file is being imported for the first time
	{
		...
		if(FAILED(ImportNode_Init(node, path))) {
			free(node);
			return NULL;
		}
		node->numRefs = 0;
