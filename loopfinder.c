...
int checkWaveValid(WAVE_HEADER* wave) {
	...
	!memcmp((char*)&wave->format, "\x01\x00", 2))
		return 1;
	else
		return 0;
}
...
size_t findWaveDataStart(WAVE_HEADER* wave) {
	if(checkWaveValid(wave)) {
		if(!memcmp((char*) &wave->formatheadersize + sizeof(wave->formatheadersize) + wave->formatheadersize, "data", 4))
			return ((size_t) &wave->formatheadersize - (size_t) wave) + sizeof(wave->formatheadersize) + wave->formatheadersize + sizeof(int);
	}
	return 0;
}
