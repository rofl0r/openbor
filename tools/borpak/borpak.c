...
int main(int argc, char *argv[]) {
	struct pnp_t *ps, *psfree;
	pn_t pn;
	FILE *fd;
	u_int off;
	int i, len, list = 0, build = 0, packver = 0, filez = 0;
	char pack[4], *dir = NULL, *patt = NULL, *fname;
...
int recursive_dir(FILE * fd, char *filedir) {
	...
	if(n < 0) {
		...
		for(i = 0; i < n; i++) {	// Changed by Plombo
			...
			sprintf(tcDir, "%s/%s", filedir, namelist[i]->d_name);
